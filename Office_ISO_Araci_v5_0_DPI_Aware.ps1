...
# ---------- YÜKSEK ÇÖZÜNÜRLÜK (DPI) FARKINDALIĞI ----------
$csharpCode = @"
using System;
using System.IO;
...
if (-not $prc.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    if ([string]::IsNullOrWhiteSpace($sp)) {
        [System.Windows.Forms.MessageBox]::Show("Bu araç .ps1 olarak kaydedilip çalıştırılmalıdır.","Başlatılamadı",'OK','Error')|Out-Null
        exit
    }
    Start-Process powershell -ArgumentList "-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File `"$sp`"" -Verb RunAs
    exit
}
